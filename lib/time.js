...
export function marketDateKey(ms) {
  if (!Number.isFinite(ms)) return null;
  return ymdInTZ(ms, MARKET_TZ);
}
...
export function validateLatestTimes(items, label, toleranceMs = 5000) {
  if (!Array.isArray(items) || items.length === 0) {
    throw new Error(`${label} upstream request incomplete: no items`);
  }

  const latestTimes = items.map((item) => item?.latestT);
  if (latestTimes.some((value) => !Number.isFinite(value))) {
    throw new Error(`${label} upstream request incomplete: missing latest timestamp`);
  }

  const dateKeys = items.map((item) => marketDateKey(item.latestT));
  const referenceDateKey = dateKeys[0];
  const mismatchedDateItems = items.filter((item, index) => dateKeys[index] !== referenceDateKey);
  if (mismatchedDateItems.length > 0) {
    const mismatchText = items
      .map((item, index) => `${item.symbol}:${dateKeys[index]}:${item.latestT}`)
      .join(", ");
    throw new Error(`${label} data date mismatch: ${mismatchText}`);
  }

  const minLatest = Math.min(...latestTimes);
  const maxLatest = Math.max(...latestTimes);
  if (maxLatest - minLatest > toleranceMs) {
    const mismatchText = items
      .map((item) => `${item.symbol}:${item.latestT}`)
      .join(", ");
    throw new Error(`${label} data timestamp mismatch: ${mismatchText}`);
  }
